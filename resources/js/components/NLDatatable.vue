        ...
        <div class="table-header grid my-6">
            ...
            <button class="btn btn-danger has-icon" @click="resetFilter">
              <i class="las la-backspace icon"></i>
                    ...
                    <template v-for="(filter, filterName) in filters">
                        <div v-if="!filter?.hide" :key="'filter-' + filterName" class="col-12"
                            :class="filter?.cols || 'col-lg-2'">
                            <NLSelect v-if="!filter.type" v-model="filter.value" :name="filterName" :label="filter.label"
                            ...
                            <div v-if="filter.type && filter.type == 'date-range'" class="grid gap-2">
                                <template v-for="(attrData, attrName) in filter.attributes">
                                    <div v-if="filter.type && filter.type === 'data-range'" :key="'filter-' + attrName"
                                        class="col-12" :class="data?.cols || 'col-lg-6'">
                                        <NLInput v-model="attrData.value" :name="attrName" :label="attrData.label"
                                            type="date" />
                                    </div>
                                </template>
                            </div>
            ...
            <div class="col-12 col-lg-8 d-flex justify-end alin-center gap-2">
                <div class="d-flex align-center">
                    <input v-if="(searchable && configLocal.data?.meta?.total) || appliedSearch !== ''" type="search"
                        class="input m-0" placeholder="Faite votre recherche..." @input="search($event.target.value)">
                </div>
        ...
        <div class="table-container">
            <table>
                <thead>
                    <tr>
                        <!-- v-if becomes filter  -->
                        <th v-for="column in isNotHiddenColumn" :key="column.field"
                            :colspan="column.colspan ? column.colspan : null" :align="column.align ? column.align : 'left'">
                ...
                <tbody v-if="configLocal.data?.meta?.total" :class="{ 'is-busy': isBusy }">
                    <tr v-for="item in configLocal.data?.data" :key="item[rowKey]">
                        <!-- v-if becomes filter  -->
                        <td v-for="column in isNotHiddenColumn" :key="column.field"
                            :colspan="column.colspan ? column.colspan : null" :align="column.align ? column.align : 'left'"
                            ...
                            <span class="d-flex justify-start align-center gap-4">
                                ...
                                <template v-for="(value, key) in configLocal.actions">
                                    <template
                                        ...
                                        <span :key="key">
                                            <button v-if="showAction(value, key, 'show', item)" class="btn btn-success"
                                                @click.stop="show(item)">
                                                <i class="las la-eye icon" />
                                            </button>
                                            <button v-if="showAction(value, key, 'edit', item)" class="btn btn-warning"
                                                @click.stop="edit(item)">
                                                <i class="las la-edit icon" />
                                            </button>
                                            <button v-if="showAction(value, key, 'delete', item)" class="btn btn-danger"
            ...
            <div class="pagination my-6 grid">
                ...
                <div v-if="showPagination()" class="col-12 col-lg-1 d-flex justify-center align-center">
                    <template v-for="(link, index) in configLocal.data?.meta?.links">
                        <button v-if="showPreviousLink(index, link.url)" :key="link.label" class="btn mx-2"
                            :class="{ 'is-active': link.active }" @click="getPaginationData(link.url)">
                            Précédent
                        </button>
                    </template>
                </div>
                ...
                <div v-if="showPagination()" class="col-12 col-lg-8 d-flex align-center justify-center">
                    <template v-for="(link, index) in configLocal.data?.meta?.links">
                        <button v-if="showPageNumberLink(index)" :key="index" class="btn is-radius mx-2"
                            :class="{ 'is-active': link.active }" :disabled="link.active"
                ...
                <div v-if="showPagination()" class="col-12 col-lg-1 d-flex justify-center align-center">
                    <template v-for="(link, index) in configLocal.data?.meta?.links">
                        <button v-if="showNextLink(index, link.url)" :key="link.label" class="btn mx-2"
                            :class="{ 'is-active': link.active }" @click="getPaginationData(link.url)">
                            Suivant
                        </button>
                    </template>
