...
async function afterEach(to, from, next) {
    // await useRouter()?.appContext?.app?.$nextTick()
    // await nextTick()
    // console.log(window.location.origin + from.full_path, window.location.href);
    let url = window?.location?.origin
    url += from?.fullPath
    localStorage.setItem('prv_url', url) // Need only to store full previous url without query string
    // router.isReady()
}
