<template>
      ...
      <div class="grid my-2 form-row" v-for="(detail, row) in details" :key="'detail-' + row">
        ...
        <div class="col-12" v-if="detail.control_point.fields">
          <div class="repeater">
            ...
            <!-- Repeater row -->
            <div class="grid my-6 repeater-row" v-for="(input, index) in setupFields(detail.control_point.fields)"
              :key="'metadata-' + index + 'row-' + row">
              <div class="col-12" v-for="(item, field) in form.rows[row].metadata">
                <div class="grid gap-2">
                  <div class="col-11">
                    <div class="grid">
                      <div :key="'metadata-input-' + input.name + '-' + field + '-id'" :class="input.style">
                        ...
                        <NLInput :form="form" :label="input.label" :placeholder="input.placeholder" :type="input.type"
                          ...
                          :name="'rows.' + row + '.metadata.' + field + '.' + input.name"
                          v-model="form.rows[row].metadata[field][input.name]" :id="'rows.' + field + '.' + input.name"
                          v-if="isInput(input.type)" />

                        <!-- {{ form['rows'][row]['metadata'][field][input.name] }} {{ row }} {{ field }} {{ input.name }} -->
                  ...
                  <div class="col-1 p-0 d-flex justify-start align-center" v-if="row >= 0">
                    <div class="btn btn-danger" @click="removeRow(row, field)">
                      Supprimer
                    </div>
                  </div>
                </div>
  ...
  <ContentBody v-can="'view_mission'" v-else>
    <button v-can="'edit_mission'" class="btn btn-warning">
      ...
      <div class="col-12" v-for="detail in details" :key="detail.id">
        <div class="box">
          <div class="grid gap-4">
            ...
            <!-- <div class="col-12">
              <div class="border-1 border-dark"></div>
            </div> -->
            ...
            <div class="col-8">{{ detail.score }}</div>
...
export default {
  ...
  methods: {
    ...
    removeRow(row, field) {
      ...
      // this.form[ this.name ].splice(row, 1);
    },
  },
}
</script>
