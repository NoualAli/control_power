        ...
        <div v-for="(detail, row) in details" :key="'detail-' + row" class="form-row">
          <div class="grid my-2">
            ...
            <div class="col-12 col-lg-8 grid">
              <!-- Major fact -->
              <div v-if="detail?.control_point.has_major_fact && [4, 3, 2].includes(form.rows[row].score)" class="col-12">
                <NLSwitch
                  v-model="form.rows[row].major_fact" :name="'rows.' + row + '.major_fact'" :form="form"
                  label="Fait majeur"
                />
              </div>
              <div class="col-12">
                <NLSelect
                  v-model="form.rows[row].score" :name="'rows.' + row + '.score'" label="Notation" :form="form"
                  :options="setupScores(detail.control_point.scores)" label-required
                />
              </div>
              ...
              <div v-if="detail.control_point.fields && form.rows[row].score > 1" class="col-12">
                <div class="repeater">
                  ...
                  <!-- Repeater row -->
                  <div
                    v-for="(item, dataRow) in form.rows[row].metadata" :key="'metadata-' + dataRow + 'row-' + row"
                    ...
                    <div class="col-12">
                      <div class="grid gap-2">
                        <div class="col-11">
                          <div class="grid">
                            <div
                              v-for="(input, index) in setupFields(detail.control_point.fields)" :key="'metadata-input-' + input.name + '-' + dataRow + '-id'"
                              :class="input.style"
                            >
                              <!-- Defining different inputs -->
                              <NLInput
                                v-if="isInput(input.type)" :id="'rows.' + row + '.metadata.' + dataRow + '.' + index + '.' + input.name" v-model="form.rows[row].metadata[dataRow][index][input.name]"
                                :form="form" :label="input.label"
                                :placeholder="input.placeholder"
                                :type="input.type"
                                :label-required="input.required"
                                :name="'rows.' + row + '.metadata.' + dataRow + '.' + index + '.' + input.name" :help-text="input.help_text"
                              />
                              ...
                              <NLTextarea
                                ...
                                :type="input.type"
                                :label-required="input.required"
                                :name="'rows.' + row + '.metadata.' + dataRow + '.' + index + '.' + input.name" :length="input.length" :help-text="input.help_text"
                              />
                              ...
                              <NLSelect
                                ...
                                :type="input.type"
                                :label-required="input.required"
                                :name="'rows.' + row + '.metadata.' + dataRow + '.' + index + '.' + input.name" :options="input.options"
                                :placeholder="input.placeholder || 'Choisissez une option...'" :multiple="input.multiple" :help-text="input.help_text"
                              />
                            </div>
                          </div>
                        </div>
            ...
            <div class="col-12 col-lg-3">
              <!-- Media (attachements) -->
              <NLFile
                v-model="form.rows[row].media" :name="'rows.' + row + '.media'" label="Pièces jointes"
                attachable-type="App\Models\MissionDetail" :attachable-id="form.rows[row].detail" :form="form" multiple
              />
            </div>
          </div>
        </div>
