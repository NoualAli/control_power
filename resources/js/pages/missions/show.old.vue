    ...
    <div class="d-flex justify-between align-items">
      <div>
        ...
        <button class="btn btn-info"
          v-if="!mission?.current.opinion && !mission?.current.dre_report && mission?.current.progress_status == 100"
          @click="showOpinion" v-can="'create_opinion'">
          Ajouter votre avis
        </button>
        <button class="btn btn-success"
          v-if="mission?.current.progress_status == 100 && !mission?.current.opinion?.is_validated && mission?.current.opinion"
        ...
        <button class="btn btn-info"
          ...
          @click="showReport" v-can="'create_dre_report'">
          Ajouter votre rapport
        </button>
        <button class="btn btn-success" v-if="mission?.current.dre_report?.is_validated" @click.prevent="showDispatchForm"
        ...
        <button class="btn btn-success"
          ...
          @click.prevent="validateReport" v-can="'validate_dre_report'">
          Valider la mission
        </button>

        <!-- Read actions -->
        <button class="btn btn-info" v-if="mission?.current.dre_report" @click="showReport" v-can="'view_dre_report'"
          v-is-current-user="mission?.current.dre_report.created_by_id">
          Rapport de la mission
        </button>
        <button class="btn btn-info" v-if="mission?.current.dre_report?.is_validated" @click="showReport"
          v-can="'view_dre_report'" v-is-not-current-user="mission?.current.dre_report.created_by_id">
          Rapport de la mission
        </button>
        <button class="btn btn-info" v-if="mission?.current.opinion?.is_validated" @click="showOpinion"
          v-can="'view_opinion'" v-is-not-current-user="mission?.current.opinion.created_by_id">
          Avis sur la mission
        </button>
        <button class="btn btn-info" v-if="mission?.current.opinion" @click="showOpinion" v-can="'view_opinion'"
          v-is-current-user="mission?.current.opinion.created_by_id">
          Avis sur la mission
        </button>
    ...
    <NLModal :show="modals.report" @close="modals.report = false">
      ...
      <template #footer>
        <button class="btn btn-success has-icon" @click.prevent="validateReport"
          v-if="!mission?.current.dre_report?.is_validated && mission?.current.dre_report && !forms.report.edit_mode"
