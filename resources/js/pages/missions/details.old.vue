<template>
  <ContentBody v-can="'control_agency,process_mission,create_dre_report,validate_dre_report'" v-if="!isCompleted">
                  ...
                  <div class="col-11">
                    <div class="grid">
                      <div :key="'metadata-input-' + input.name + '-' + dataRow + '-id'" :class="input.style"
                        ...
                        <NLTextarea :form="form" :label="input.label" :placeholder="input.placeholder" :type="input.type"
                          :labelRequired="input.required"
                          :name="'rows.' + row + '.metadata.' + dataRow + '.' + index + '.' + input.name"
                          v-model="form.rows[row].metadata[dataRow][index][input.name]"
                          :id="'rows.' + row + '.metadata.' + dataRow + '.' + index + '.' + input.name"
  ...
  <div v-can="'view_mission_detail'" v-else>
    <ContentHeader>
      ...
      <template #actions>
        <button v-can="'edit_mission,control_agency,create_dre_report,validate_dre_report'" class="btn btn-warning my-6"
          @click="switchToExeMode()" v-if="!mission?.dre_report?.is_validated">
          <i class="las la-pen"></i>
        </button>
        <button v-can="'process_mission'" v-if="!mission?.cdcr_validation_at" class="btn btn-warning my-6"
          @click="switchToExeMode()">
  ...
  methods: {
    ...
    initData(isCompleted = true) {
      this.$store.dispatch('details/fetchConfig', { missionId: this.$route.params.missionId, processId: this.$route.params.processId }).then(() => {
        ...
        this.isCompleted = this.details[ 0 ].executed_at !== '-' && isCompleted
        if (!this.isCompleted) {
          this.initForm()
        }
      })
