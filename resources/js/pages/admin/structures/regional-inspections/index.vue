    ...
    <ContentBody v-if="can('view_regional_inspection')">
        <NLDatatable :columns="columns" :actions="actions" title="Liste des inspections régionales"
            ...
            <template #actions-before="{ item }">
                <a class="btn btn-office-excel" :href="'/excel-export?export=regional-inspections&id=' + item.id"
                    target="_blank">
                    <NLIcon name="table" />
                </a>
            </template>
            ...
            <template #table-actions>
                <router-link v-if="can('create_regional_inspection')" :to="{ name: 'regional-inspection-create' }"
                    class="btn has-icon">
                    <NLIcon name="add" />
                    Ajouter
                </router-link>
...
export default {
    ...
    metaInfo() {
        return { title: 'Inspections régionales' }
    },
    data() {
        return {
            currentUrl: null,
            ...
            actions: {
                edit: {
                    show: (item) => {
                        return this.can('edit_regional_inspection')
                    },
                    apply: this.edit
                },
                delete: {
                    show: (item) => {
                        return this.can('delete_regional_inspection')
                    },
                    apply: this.destroy
                },
            }
    ...
    methods: {
         ...
         */
        edit(event) {
            this.$router.push({ name: 'regional-inspection-edit', params: { regionalInspection: event.item.id } })
        },
        ...
        destroy(event) {
            this.$swal.confirm_destroy().then((action) => {
                if (action.isConfirmed) {
                    api.delete('structures/regional-inspections/' + event.item.id).then(response => {
