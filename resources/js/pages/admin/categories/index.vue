<template>
    <div v-if="can('view_category')">
        ...
        <ContentBody>
            <NLDatatable :config="config" title="Liste des catégories" namespace="categories" @show="show" @delete="destroy"
                @edit="edit" />
            <NLModal :show="rowSelected" @close="rowSelected = null">
                ...
                <template #footer>
                    <div v-if="can('delete_category,edit_category')" class="d-flex justify-end align-center gap-5 w-100">
                        <button v-if="can('delete_category')" class="btn btn-danger has-icon"
                        ...
                        <button v-if="can('edit_category')" class="btn btn-warning has-icon"
