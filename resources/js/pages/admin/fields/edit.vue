...
<template>
    <div v-if="can('edit_field')">
        <ContentHeader :title="'Modifier le champ ' + field?.current?.label" />
        <ContentBody>
...
export default {
    ...
    methods: {
        ...
        initData() {
            ...
            this.$store.dispatch('fields/fetch', this.$route.params.field).then(() => {
         ...
         */
        update() {
            this.form.put('fields/' + this.field?.current?.id).then(response => {
