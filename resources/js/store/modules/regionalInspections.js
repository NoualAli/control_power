...
export const actions = {
    async fetchAll({ commit }) {
        const url = 'structures/regional-inspections?fetchAll'
        const { data } = await api.get(url)
        commit('FETCH_ALL', { all: data })
    },
    async fetchPaginated({ commit }) {
        const { data } = await api.get('structures/regional-inspections')
        commit('FETCH_PAGINATED', { paginated: data })
    },
    async fetch({ commit }, id) {
        try {
            const { data } = await api.get('structures/regional-inspections/' + id)
