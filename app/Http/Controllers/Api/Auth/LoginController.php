...
class LoginController extends Controller
{
    ...
    protected function attemptLogin(Request $request): bool
    {
        ...
        if (!$token) {
            EventLog::store(['type' => EventLogTypes::LOGIN, 'user_full_name' => $request->authLogin, 'payload' => ['success' => false]]);
            return false;
        }
    ...
    protected function storeLoginInformations(Request $request, User $user)
    {
        try {
            return DB::transaction(function () use ($user, $request) {
                $loginInformationsStored = Login::firstOrCreate([
                    'user_id' => $user->id,
                    'ip_address' => $request->ip(),
                    'user_agent' => $request->userAgent(),
                    'last_activity' => now(),
                ]);
                $eventLogStored = EventLog::store(['type' => EventLogTypes::LOGIN, 'payload' => ['success' => true]]);

                return $loginInformationsStored;
            });
        } catch (\Throwable $th) {
    ...
    protected function updateLastLoginInformations(Request $request, User $user)
    {
        try {
            return DB::transaction(function () use ($user, $request) {
                $loginInformationsUpdated = Login::where('user_id', $user->id)
                ...
                $eventLogStored = EventLog::store(['type' => EventLogTypes::LOGOUT, 'payload' => ['success' => $loginInformationsUpdated]]);
                return $loginInformationsUpdated;
            });
        } catch (\Throwable $th) {
            return throwedError($th);
        }
