...
class AccomplishRateExport extends BaseExport implements FromView, WithProperties, ShouldAutoSize
{
    private $data;
    ...
    {
        EventLog::store(['type' => EventLogTypes::EXPORT, 'attachable_type' => User::class, 'payload' => ['content' => 'KPI accomplishRate']]);
        $data = $this->data;
        return view('export.kpi.accomplish_rate', compact('data'));
    }
        ...
        for ($row = 2; $row <= $sheet->getHighestRow(); $row++) {
            for ($colIndex = 10; $colIndex <= $highestColumnIndex; $colIndex++) {
                ...
                if ($cellValue > 75) {
                    ...
                    $style['font']['color']['argb'] = 'FFFFFFFF';
                } elseif ($cellValue > 50 && $cellValue < 75) {
