    ...
    public function achievementRate(): array
    {
        ...
        foreach ($missions as $dre => $missions) {
            $dreMissions = DB::table('dres as d')
                ->select(
                    DB::raw("CONCAT(d.code, ' - ', d.name) as dre_name"),
                    DB::raw("CONCAT(a.code, ' - ', a.name) as agency_name"),
                    'm.reference',
                )
            ...
            $anomaliesCount = (clone $dreMissions)->leftJoin('mission_details AS md', 'md.mission_id', 'm.id')->whereIn('score', [2, 3, 4])->count();
            $rate = $total ? number_format(($totalAchieved * 100) / $total, 2) : 0;
            array_push($achievments, ['dre' => $dre, 'total' => $total, 'totalAchieved' => $totalAchieved, 'rate' => $rate, 'anomalies_count' => $anomaliesCount]);
        }
